...
class RealtimeConversation:
    ...
    def _process_item_created(self, event: Dict, *args) -> Tuple[Dict, None]:
        ...
        if new_item.get('type') == 'message':
            if new_item.get('role') == 'user':
                ...
                if new_item.get('content'):
                    for i, content in enumerate(new_item['content']):
    ...
    def _process_input_audio_transcription_completed(self, event: Dict, *args) -> Tuple[Optional[Dict], Optional[Dict]]:
    ...
    def _process_response_output_item_done(self, event: Dict, *args) -> Tuple[Optional[Dict], None]:
    ...
    def _process_response_content_part_added(self, event: Dict, *args) -> Tuple[Optional[Dict], None]:
    ...
    def _process_response_audio_delta(self, event: Dict, *args) -> Tuple[Dict, Dict]:
        ...
        item['formatted']['audio'] = RealtimeUtils.merge_int16_arrays(
